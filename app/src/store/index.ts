...
export default createStore({
  ...
  actions: {
    ...
    async datesLoad(context: ActionContext<StoreState, StoreState>): Promise<DateInfo[]> {
      const res = await fetcher<string[]>('GET', '/api/flight');
      const dates: DateInfo[] = (res.data ?? []).map(d => ({
        text: DateTime.fromISO(d).toLocaleString(DateTime.DATETIME_SHORT_WITH_SECONDS),
        value: d
      }));
      // reverse sort by date
      dates.sort((a: DateInfo, b: DateInfo) => a.value > b.value ? 1 : -1);
      context.commit('dates', dates);
      return dates;
    },

    async flightDateLoad(context: ActionContext<StoreState, StoreState>, date: string): Promise<Flight[]> {
      const res = await fetcher<Flight[]>('GET', `/api/flight/${date}`);
      const flights = res.data ?? [];
      context.commit('flightDates', { date, flights });
      return flights;
    },

    async countriesLoad(context: ActionContext<StoreState, StoreState>): Promise<Country[]> {
    ...
    async countryDatesLoad(context: ActionContext<StoreState, StoreState>, date: string): Promise<CountryCount[]> {
      const res = await fetcher<CountryCount[]>('GET', `/api/country/${date}`);
      const countries = res.data ?? [];
  ...
  mutations: {
    ...
    flightDates: (state: StoreState, { date, flights }: {date: string; flights: Flight[]}) => {
    ...
    countryDates: (state: StoreState, { date, countries }: {date: string; countries: CountryCount[]}) => {
  ...
  getters: {
    ...
    flightDate: (state: StoreState) => (date: string): Flight[] | undefined => {
      return state.flightDates[date];
    },

    countryDate: (state: StoreState) => (date: string): CountryCount[] | undefined => {
